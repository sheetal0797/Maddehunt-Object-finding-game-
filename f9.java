import javax.swing.*;
import java.awt.*;
import java.awt.event.*;
import java.io.*;

class showhintf9 extends Thread
{
JLabel frm23;
public showhintf9(JLabel obj)
{
frm23=obj;
}

public void run()
{
try
{
for(int i=0;i<10;i++)
{
frm23.setVisible(!frm23.isVisible());
Thread.sleep(230);
}
frm23.setVisible(true);
}
catch(Exception ex)
{
System.out.println(ex.getMessage());
}
}
}
//-----------------------------------------------
class showtimef9 implements Runnable
{
f9 frm23;
int time;
JLabel lb25;
public showtimef9(f9 obj,JLabel object,int tm)
{
frm23=obj;
time=tm;
lb25=object;
}

public void run()
{
try
{
int a2=9;
String z2=null;  
FileInputStream fin;
	try
		{
		    fin = new FileInputStream("newfile.txt");
      z2=new DataInputStream(fin).readLine();
		    fin.close();		
		}
		catch (IOException e)
		{
			System.err.println ("Unable to read from file");
			System.exit(-1);
		}
int foo = Integer.parseInt(z2);
if(foo<9)
{
FileOutputStream fout;		

		try
		{
		    fout = new FileOutputStream ("newfile.txt");
		    new PrintStream(fout).print(a2);
		    fout.close();		
		}
		catch (IOException e)
		{
			System.err.println ("Unable to write to file");
			System.exit(-1);
	}	}
int a=time*600;
for(int i=(time-1);i>=0;i--)
{for(int j=60;j>=0;j--)
{lb25.setText(""+i+":"+j+"");
 lb25.setFont(new Font("Algerian", Font.BOLD,30));
      lb25.setForeground(Color.white);
Thread.sleep(1000);}
}
}
catch(Exception ex)
{
System.out.println(ex.getMessage());
}
if(frm23.i<15)
{JOptionPane.showMessageDialog(null,"GAME OVER...");
new f9N();
frm23.dispose();}
}
}
//-------------------------------------------------------------------


class f9 extends JFrame implements MouseListener
{
    JLabel label,label1,label2,an,label3,label4,label5,label6,label7,label8,label9,label10,label11,label12,label13,label14,l,u,v,w,a,b,c,h1,h2,h3,c1,c2,c3;
    int i,k15,k16;
    boolean dt;
showhintf9 sh23;
Thread t1; 
 Thread th;
    public f9(int k14) 
  {     setSize(1279,767);
        setVisible(true);
	//setDefaultCloseOperation(EXIT_ON_CLOSE);
        setResizable(false);	
        JPanel panel = (JPanel)getContentPane();  
        panel.setLayout(null);
        l=new JLabel();
u = new JLabel();
	h1 = new JLabel();  
        h2= new JLabel(); 
        h3 = new JLabel();
    an = new JLabel("Show names again");
c1 = new JLabel();
	c2 = new JLabel();
	c3 = new JLabel();
c1.setIcon(new ImageIcon("obj/c6.png"));
 h1.setIcon(new ImageIcon("h1.png"));
        h2.setIcon(new ImageIcon("h2.png"));
	h3.setIcon(new ImageIcon("h3.png"));
	label = new JLabel();  
        label1 = new JLabel(); 
        label2 = new JLabel(); 
	label3 = new JLabel(); 
	label4 = new JLabel();  
        label5 = new JLabel(); 
        label6 = new JLabel(); 
	label7 = new JLabel();
        label8 = new JLabel();  
        label9 = new JLabel(); 
        label10 = new JLabel(); 
	label11 = new JLabel(); 
	label12 = new JLabel(); 
	label13 = new JLabel();
	label14 = new JLabel();
     
        label.setIcon(new ImageIcon("obj/candle1.png"));
        label1.setIcon(new ImageIcon("obj/eye.png"));
	label2.setIcon(new ImageIcon("obj/cotton bud.png"));
	label3.setIcon(new ImageIcon("obj/toothpaste.png"));
        label4.setIcon(new ImageIcon("obj/ashok symbol.png"));
        label5.setIcon(new ImageIcon("obj/fountain pen.png"));
	label6.setIcon(new ImageIcon("obj/apple sign.png"));
	label7.setIcon(new ImageIcon("obj/cup.png"));
        label8.setIcon(new ImageIcon("obj/cattle.png"));
        label9.setIcon(new ImageIcon("obj/ring.png"));
	label10.setIcon(new ImageIcon("obj/brifcase4.png"));
	label11.setIcon(new ImageIcon("obj/mashaal.png"));
	label12.setIcon(new ImageIcon("obj/hand.png"));
	label13.setIcon(new ImageIcon("obj/basket1.png"));
	label14.setIcon(new ImageIcon("obj/currency coin.png"));
     panel.add(c1);c1.addMouseListener(this);
     panel.add(c2);
     panel.add(c3);
     panel.add(an);an.addMouseListener(this);
         panel.add(label); label.addMouseListener(this);
         panel.add(label1); label1.addMouseListener(this);
	 panel.add(label2); label2.addMouseListener(this);
	 panel.add(label3); label3.addMouseListener(this);
         panel.add(label4);label4.addMouseListener(this);
         panel.add(label5); label5.addMouseListener(this);
	 panel.add(label6); label6.addMouseListener(this);
	 panel.add(label7); label7.addMouseListener(this);
         panel.add(label8); label8.addMouseListener(this);
         panel.add(label9);label9.addMouseListener(this);
	 panel.add(label10);label10.addMouseListener(this);
	 panel.add(label11); label11.addMouseListener(this);
	 panel.add(label12); label12.addMouseListener(this);
	 panel.add(label13); label13.addMouseListener(this);
	 panel.add(label14); label14.addMouseListener(this);
panel.add(h1); h1.addMouseListener(this);
        panel.add(h2); h2.addMouseListener(this);
	panel.add(h3); h3.addMouseListener(this);
	   Dimension size = label.getPreferredSize();
   c1.setBounds(1200, 665, size.width+150, size.height+23);
          c2.setBounds(379, 0, size.width+80, size.height);
          c3.setBounds(946, 0, size.width+140, size.height);
  h1.setBounds(10, 613, 100,100);
	 h2.setBounds(30, 613, 100,100);	
          h3.setBounds(50, 613, 100,100);
  an.setBounds(450, 0, 100,100);

          label.setBounds(587, 0, size.width+80, size.height+80);
	  label1.setBounds(296, 104, size.width+80, size.height+80);	
          label2.setBounds(33, 78, size.width+80, size.height+80);
	  label3.setBounds(752, 230, size.width+80, size.height+80);
	  label4.setBounds(186, 50, size.width+80, size.height+80);
	  label5.setBounds(804, 566, size.width+110, size.height+110);	
          label6.setBounds(191, 431, size.width+80, size.height+80);
	  label7.setBounds(440, 136, size.width+80, size.height+80);
	  label8.setBounds(85, 240, size.width+80, size.height+80);
	  label9.setBounds(786, 80, size.width+80, size.height+80);          
          label10.setBounds(746, 100, size.width+80, size.height+80);
	  label11.setBounds(1237, 67, size.width+80, size.height+80);
	  label12.setBounds(980, 114, size.width+100, size.height+100);
	  label13.setBounds(370, 270, size.width+80, size.height+80);
	  label14.setBounds(460, 265, size.width+80, size.height+80);
          setLayout(new BorderLayout());
        JLabel background=new JLabel(new ImageIcon("9.jpg"));
        add(background);
	i=0;
k15=k14;
k16=0;
c3.setText(""+(i*100)+"Points");
 c3.setFont(new Font("algerian", Font.ITALIC,23));
      c3.setForeground(Color.black);
	pack();
showtimef9 t=new showtimef9(this,c2,2);
	th=new Thread(t);
	th.start();
framedisplay fd=new framedisplay("Candle,Eye,Cotton,Toothpaste,Ashok Symbol,Pen,Apple Symbol,Cup,Cattle,Ring,Briefcase,Mashaal,Hand,Basket,Currency Coin");
t1=new Thread(fd);
 t1.start(); 
}
public void mouseEntered(MouseEvent ae){}
public void mouseExited(MouseEvent ae){}
public void mousePressed(MouseEvent ae){}
public void mouseReleased(MouseEvent ae){}
public void mouseClicked(MouseEvent ae)
{ 
/*if(ae.getSource()==an)
{t1.start();
}*/

 if(ae.getSource()==label)
{i++;checkscore(this);
label.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}
  	
else if(ae.getSource()==label1)
{i++;checkscore(this);
label1.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}
  	
else if(ae.getSource()==label2)
{i++;checkscore(this);
label2.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}

else if(ae.getSource()==label3)
{i++;checkscore(this);
label3.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}

else if(ae.getSource()==label4)
{i++;checkscore(this);
label4.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}
  	
else if(ae.getSource()==label5)
{i++;checkscore(this);
label5.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}
  	
else if(ae.getSource()==label6)
{i++;checkscore(this);
label6.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}
  	
else if(ae.getSource()==label7)
{i++;checkscore(this);
label7.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}
  	
else if(ae.getSource()==label8)
{i++;checkscore(this);
label8.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}
  	
else if(ae.getSource()==label9)
{i++;checkscore(this);
label9.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}
  	
else if(ae.getSource()==label10)
{i++;checkscore(this);
label10.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}
  	
else if(ae.getSource()==label11)
{i++;checkscore(this);
label11.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}

else if(ae.getSource()==label12)
{i++;checkscore(this);
label12.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}

else if(ae.getSource()==label13)
{i++;checkscore(this);
label13.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}

else if(ae.getSource()==label14)
{i++;checkscore(this);
label14.setVisible(false);check(this);
hintsong d2=new hintsong();
	Thread e2=new Thread(d2);
	e2.start();}

else if(ae.getSource()==c1)
{new fmenu((JFrame)this,0);
th.stop();
}

else if(ae.getSource()==h1)
{if(label.isVisible()==true)
   {u=label;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label1.isVisible()==true)
   {u=label1;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label2.isVisible()==true)
   {u=label2;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label3.isVisible()==true)
   {u=label3;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label4.isVisible()==true)
  {u=label4;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label5.isVisible()==true)
   {u=label5;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label6.isVisible()==true)
   {u=label6;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label7.isVisible()==true)
   {u=label7;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label8.isVisible()==true)
   {u=label8;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label9.isVisible()==true)
   {u=label9;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label10.isVisible()==true)
   {u=label10;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label11.isVisible()==true)
   {u=label11;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label12.isVisible()==true)
   {u=label12;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label13.isVisible()==true)
   {u=label13;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label14.isVisible()==true)
   {u=label14;
sh23=new showhintf9(u);
	sh23.start();  }

h1.setVisible(false);}

else if(ae.getSource()==h2)
{if(label.isVisible()==true)
   {u=label;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label1.isVisible()==true)
   {u=label1;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label2.isVisible()==true)
   {u=label2;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label3.isVisible()==true)
   {u=label3;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label4.isVisible()==true)
  {u=label4;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label5.isVisible()==true)
   {u=label5;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label6.isVisible()==true)
   {u=label6;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label7.isVisible()==true)
   {u=label7;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label8.isVisible()==true)
   {u=label8;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label9.isVisible()==true)
   {u=label9;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label10.isVisible()==true)
   {u=label10;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label11.isVisible()==true)
   {u=label11;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label12.isVisible()==true)
   {u=label12;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label13.isVisible()==true)
   {u=label13;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label14.isVisible()==true)
   {u=label14;
sh23=new showhintf9(u);
	sh23.start();  }

h2.setVisible(false);}

else if(ae.getSource()==h3)
{if(label.isVisible()==true)
   {u=label;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label1.isVisible()==true)
   {u=label1;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label2.isVisible()==true)
   {u=label2;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label3.isVisible()==true)
   {u=label3;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label4.isVisible()==true)
  {u=label4;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label5.isVisible()==true)
   {u=label5;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label6.isVisible()==true)
   {u=label6;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label7.isVisible()==true)
   {u=label7;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label8.isVisible()==true)
   {u=label8;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label9.isVisible()==true)
   {u=label9;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label10.isVisible()==true)
   {u=label10;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label11.isVisible()==true)
   {u=label11;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label12.isVisible()==true)
   {u=label12;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label13.isVisible()==true)
   {u=label13;
sh23=new showhintf9(u);
	sh23.start();  }
else if(label14.isVisible()==true)
   {u=label14;
sh23=new showhintf9(u);
	sh23.start();  }
h3.setVisible(false);}


     }

    public static void main(String args[]) 
    {
	f9 bgf9=new f9(0);
         //SwingUtilities.invokeLater(new Runnable() {  public void run() {  }; });  
     int t;  
int a2=9;
String z2=null;  
FileInputStream fin;
	try
		{
		    fin = new FileInputStream("newfile.txt");
      z2=new DataInputStream(fin).readLine();
		    fin.close();		
		}
		catch (IOException e)
		{
			System.err.println ("Unable to read from file");
			System.exit(-1);
		}
int foo = Integer.parseInt(z2);
if(foo<9)
{
FileOutputStream fout;		

		try
		{
		    fout = new FileOutputStream ("newfile.txt");
		    new PrintStream(fout).print(a2);
		    fout.close();		
		}
		catch (IOException e)
		{
			System.err.println ("Unable to write to file");
			System.exit(-1);
	}	}
check(bgf9);
}

public static void checkscore(f9 bgf9)
{int i,k15;
k15=bgf9.k15;
bgf9.c3.setText(""+(k15+(bgf9.i*100))+"Points");
 bgf9.c3.setFont(new Font("algerian", Font.ITALIC,23));
      bgf9.c3.setForeground(Color.black);
bgf9.k16=k15+(bgf9.i*100);
}
public static void check(f9 bgf9)
{
int i,j,a;
if(bgf9.i==15)
{
//JOptionPane.showMessageDialog(null,"Congratulations!!! You Have Cleared Stage Successfully.");
a=0;

if(bgf9.h1.isVisible()==true)
{
JOptionPane.showMessageDialog(null,"Congratulations!!! hint bonus:100 points");
try
{for(j=0;j<=100;j++)
{
bgf9.c3.setText(""+(j+bgf9.k16+a)+"Points");
Thread.sleep(5);}
}
catch(Exception ex){JOptionPane.showMessageDialog(null,ex.getMessage());}
a=a+100;
bgf9.h1.setVisible(false);}


if(bgf9.h2.isVisible()==true)
{JOptionPane.showMessageDialog(null,"Congratulations!!! hint bonus:100 points");

try
{for(j=0;j<=100;j++)
{
bgf9.c3.setText(""+(j+bgf9.k16+a)+"Points");
Thread.sleep(5);}
}
catch(Exception ex){JOptionPane.showMessageDialog(null,ex.getMessage());}
a=a+100;
bgf9.h2.setVisible(false);}


if(bgf9.h3.isVisible()==true)
{JOptionPane.showMessageDialog(null,"Congratulations!!! hint bonus:100 points");
try
{for(j=0;j<100;j++)
{
bgf9.c3.setText(""+(j+bgf9.k16+a)+"Points");
Thread.sleep(5);}
}
catch(Exception ex){JOptionPane.showMessageDialog(null,ex.getMessage());}

a=a+100;
bgf9.h3.setVisible(false);}
bgf9.k16=bgf9.k16+a;
JOptionPane.showMessageDialog(null,"Total Score:"+bgf9.k16);
String r1=null;  
FileInputStream fin;

	try
		{
		    fin = new FileInputStream("score.txt");
      r1=new DataInputStream(fin).readLine();
		    fin.close();		
		}
		catch (IOException e)
		{
			System.err.println ("Unable to read from file");
			System.exit(-1);
		}
//if(z1)
int fo = Integer.parseInt(r1);
if(fo<bgf9.k16)
{String s;
 
     
 s= JOptionPane.showInputDialog("Enter your name ");
      
FileOutputStream foutput;		

		try
		{
		    foutput = new FileOutputStream ("score name.txt");
		    new PrintStream(foutput).print(s);
		    foutput.close();		
		}
		catch (IOException e)
		{
			System.err.println ("Unable to write to file");
			System.exit(-1);
		}
FileOutputStream fout;		

		try
		{
		    fout = new FileOutputStream ("score.txt");
		    new PrintStream(fout).print(bgf9.k16);
		    fout.close();		
		}
		catch (IOException e)
		{
			System.err.println ("Unable to write to file");
			System.exit(-1);
	}	}
new f13(bgf9.k16);
bgf9.dispose();
}
 }

}